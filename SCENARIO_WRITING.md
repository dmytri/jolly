...
A `@property` invariant may assert a **testable conformance property of the verification layer itself** — e.g. "the suite uses no forbidden test double" (feature 026). The bar is unchanged: it must be a falsifiable check on observable state, not aspiration. **Testability, not subject, decides what is a scenario** — an invariant whose subject is the harness is admissible exactly when it is checkable; an untestable aspiration ("the suite should be clean") is not. The only admissible test double is one whose scenario is tagged `@exceptional-double` — an exceptional condition the real test env cannot produce on demand (an org at its environment limit; a deliberately unreachable service for a "stored, not verified" path) — and the conformance invariant permits doubles only at those sites.
