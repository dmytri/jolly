...
- **Provision instead of skip.** When a sandbox run needs a Saleor endpoint or app token that is not configured and `JOLLY_SALEOR_CLOUD_TOKEN` is present, the harness provisions an environment under the per-run `jolly-cannon-fodder` namespace (passed via the `--name`/`--domain-label` overrides), derives the missing `NEXT_PUBLIC_SALEOR_API_URL`/`JOLLY_SALEOR_APP_TOKEN` from it, and registers its teardown **before** creating — so a run never permanently consumes a sandbox slot even if it times out or crashes.
- **Concurrency: per-worker isolated stores plus a heavy/light phase split.** The free test org holds two concurrent environments, and each parallel worker provisions its own `jolly-cannon-fodder`-namespaced store, so the store name carries the worker id and no two workers ever share a store. Every per-run resource a parallel worker creates carries the worker id, the Saleor environment and the Vercel project alike, so one worker's teardown never deletes another worker's live deploy. Isolation removes cross-worker collision, but two constraints are LOCAL, not Saleor capacity, and they are why heavy scenarios run serial. First, this test VM is resource-limited: each heavy scenario runs a full toolchain (`git clone` Paper, `pnpm install` a whole Next.js app, `@saleor/configurator` deploy, `npx vercel` deploy, node), and two of those at once saturate the VM's CPU, memory, and network, which is where the "unable to connect" errors come from. Second, a freshly-provisioned Saleor environment answers 404 / 503 until its store instance is serving (cold start), and the provisioner hands off before polling for readiness, so a heavy scenario can race a not-yet-serving store, a window the VM contention widens. Neither is Saleor being saturated by two requests, and neither is fixed by a paid instance. Therefore the tier is a heavy/light split. HEAVY scenarios (a full `jolly start`, a real deploy, or a store provision, tagged `@heavy`) run SERIAL, since one toolchain fits the VM and a single stream lets each env warm. The env-creating scenarios (feature 012 create, feature 026 reclaim, tagged `@creates-env`) also run serial, since they need a slot the two isolated worker envs would consume. Only the light query and check scenarios run in parallel across the two isolated worker envs. The real robustness fix for the cold-start races is a readiness gate: poll the provisioned store until it serves before handing off.
- **Leftover handling.** Before creating, a `BeforeAll` reclaims leftover `jolly-cannon-fodder`-namespaced environments from previous runs, keyed off **this** run's id so a live sibling worker's environment is never deleted. They are this test org's disposable resources, positively identified by the `jolly-cannon-fodder-` prefix. It never deletes an environment lacking that prefix. Each worker's isolated environment is torn down at run end.
- **Skip-not-fail conditions.** Tests are skipped (not failed), with a clear reason, only when needed credentials are absent and cannot be derived (`JOLLY_SALEOR_CLOUD_TOKEN` itself, or the third-party Vercel credential), or a needed capability is unavailable (e.g. no `vercel login` session for a live deploy). An org environment-limit rejection is NOT a skip: this is a dedicated test org, so the harness reclaims capacity by deleting `jolly-cannon-fodder`-namespaced environments and proceeds.
