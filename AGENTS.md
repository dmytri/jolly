...
- **Concurrency: per-worker isolated stores, capped at the env budget, with a serial env-creating tail.** The free test org holds two concurrent environments. The bulk store-touching scenarios run in parallel with **one isolated environment per worker**: each worker provisions and owns its own `jolly-cannon-fodder`-namespaced store, so the store name carries the worker id and no two workers ever share a store. Isolated stores are the fix for concurrent load; a single shared store forced every worker's `jolly start` deploys onto one instance until it went not-serving, which reads as an instance limit but is store contention. Because each isolated worker holds one of the two slots, the bulk worker count is capped at 2. Every per-run resource a parallel worker creates carries the worker id, the Saleor environment and the Vercel project alike, so one worker's teardown never deletes another worker's live deploy. The few env-creating scenarios (feature 012 create and collision, feature 026 reclaim) are tagged `@creates-env` and run in a **serial second phase** after the bulk phase releases its environments, because they need a free slot the parallel bulk consumes. Serial is the fallback only when a run cannot isolate; a paid instance is not required.
