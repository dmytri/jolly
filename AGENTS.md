...
This repository uses Shipshape for agent workflow. Shipshape owns the generic `/captain`, `/qm`, `/crew`, `/boatswain`, `/shipwright`, and `/clearrole` role prompts; do not recreate them locally.
...
On any role takeover (QM→Captain, Crew→QM, Boatswain→Captain, and so on), the **outgoing role's final-report blockers and open questions are the FIRST agenda item** the incoming role enumerates and addresses — before discovery, spec maintenance, dirty-deck routing, or any outbound/push decision. Read the immediately-preceding role's final report verbatim and treat its blocker/open-question list as the agenda anchor.

- A single takeover is often **several situations at once** (e.g. post-Boatswain completed work AND an unresolved blocker report); do not classify it into one bucket and silently drop the rest. Handle the blockers first, then the other applicable paths.
...
- **Concurrency: none. Every tier runs serial.** No profile configures a worker count and `RIGGING.md` declares none; cucumber is serial by default, so this is the absence of configuration rather than a setting. All scenarios of a run share ONE provisioned store, coordinated through a lock and a run-scoped state file, so there is one live shared store per run. The `@sandbox` tier still runs as two sequential legs, and the split is ORDER, not concurrency: the licensed `@pipeline` and `@creates-env` scenarios run first because they build the shared state the second leg asserts against. Parallel workers were retired on 2026-07-22 after two kernel out-of-memory kills; the cause was fifteen support modules each building their own TypeScript project, since fixed by sharing one, but the tier stays serial regardless. Cold start is a SEPARATE problem and its fix is a readiness gate, which works identically either way.
...
Prefer fast focused checks and isolated slow checks. Every tier runs serial: `cucumber-js -p logic` and the rest configure no worker count. Cached verification may be used only when project tooling defines it; reports must distinguish fresh results from cache-backed results.
...
- A PTY/loopback timing race — make the target end its waits on an observed signal rather than a guessed delay. A target that only passes on a quiet machine is not yet fixed.
...
- **Reports unreachable production code; does not remove it.** `src/` is disposable and exists only to satisfy current specs and tests, so a path no current scenario, test, or step exercises is dead. Detect it by following from the current verification surface and report it. Removal is harbour work under Shipwright, per the current Shipshape Articles: Boatswain does not delete production code. When it is unclear whether a path is truly unreachable or covers live behaviour the tests merely under-exercise, say so rather than guessing. *(Corrected 2026-07-22: the previous wording instructed Boatswain to remove it, which the Articles forbid.)*
...
- Boatswain may remove assets only when specs retire them.
