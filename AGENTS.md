...
- **Provision the shared store — cached across invocations, not just within a run.** Most `@sandbox` scenarios need a live store but do not themselves test store creation; for those, the harness provisions ONE environment under a STABLE `jolly-cannon-fodder-shared` name (not a per-run id), reused across cucumber invocations via Jolly's own reuse-by-label logic (feature 012) whenever it is still reachable, and self-heals by deleting and recreating it if it isn't. It is deliberately never torn down — persisting it is the point, since it cuts the minutes-long create+deploy cost every run that doesn't test creation would otherwise pay again. The one carve-out from "every created resource is torn down": this stable-named store, exempted from every reclaim pass by name. Scenarios that DO test creation/reclaim itself (`@creates-env`) provision their own disposable, run-namespaced environment and tear it down as before. The Cloud token that drives provisioning is present by fitting-out.
