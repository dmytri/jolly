...
- **Concurrency: per-worker isolated stores plus a heavy/light phase split.** The free test org holds two concurrent environments, and each parallel worker provisions its own `jolly-cannon-fodder`-namespaced store, so the store name carries the worker id and no two workers ever share a store. Every per-run resource a parallel worker creates carries the worker id, the Saleor environment and the Vercel project alike, so one worker's teardown never deletes another worker's live deploy. Isolation removes cross-worker COLLISION but not concurrent LOAD: measured against the free org, two workers each running a full `jolly start` (provision, configurator deploy, storefront, Vercel) drive the instance to sustained not-serving (HTTP 503 / "unable to connect"), which no retry budget rides out, so it is the free instance's concurrent-load ceiling, not a store-contention artifact. Therefore the tier is a heavy/light split. HEAVY scenarios (a full `jolly start`, a real deploy, or a store provision, tagged `@heavy`) run SERIAL, since the free instance sustains exactly one heavy stream. The env-creating scenarios (feature 012 create, feature 026 reclaim, tagged `@creates-env`) also run serial, since they need a slot the two isolated worker envs would consume. Only the light query and check scenarios run in parallel across the two isolated worker envs. Real parallel speedup on the heavy bulk needs a paid instance; on the free org, serial-heavy is the reliable point.
