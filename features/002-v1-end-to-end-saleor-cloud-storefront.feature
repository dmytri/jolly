Feature: V1 end-to-end Saleor Cloud storefront setup
  ...
  Rule: V1 operational readiness
    ...
    - On a fully completed run, the agent (`--json`) success envelope LEADS its `nextSteps` with a strong, unmissable directive to restart the agent — required, not optional — because the installed skills only register on a fresh session and Jolly's exit guidance drifts out of context otherwise. It then carries a "keep building" next step orienting the agent to what setup left on disk — the `storefront/` repo (developed with `npx pnpm dev` / `npx vercel`, driven by `storefront/AGENTS.md` and the embedded `saleor-paper-storefront` skill first, with `storefront-builder` as the framework-agnostic fallback) and `recipe.yml` (the store's catalog/config as code, previewed with `npx @saleor/configurator diff` and applied with `deploy --failOnDelete`, the `saleor-configurator` skill owning the schema) — and notes that a new Saleor app is installed by the agent itself via the staff-authenticated `appInstall` mutation (using the `SALEOR_TOKEN` Jolly provisioned), with reference links. Both are success-only, never appearing on a paused, blocked, or failed run.
