...
Given(lit("Jolly has a candidate Saleor GraphQL endpoint"), function (this: JollyWorld) {
  const endpoint = process.env.JOLLY_SALEOR_URL;
...
Then(
  ...
  function (this: JollyWorld) {
    ...
    const host = new URL(process.env.JOLLY_SALEOR_URL as string).host;
    assert.ok(
...
When(lit("Jolly needs credentials for Configurator or privileged Saleor operations"), async function (this: JollyWorld) {
  ...
  if (process.env.JOLLY_SALEOR_APP_TOKEN) {
...
Then(
  ...
  function (this: JollyWorld) {
    ...
    if (!withToken) return "skipped" as const; // JOLLY_SALEOR_APP_TOKEN absent
