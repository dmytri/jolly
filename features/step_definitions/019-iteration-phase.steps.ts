...
Given(lit("jolly init has completed"), LONG, async function (this: JollyWorld) {
  ...
  writeFileSync(
    ...
    [
      `NEXT_PUBLIC_SALEOR_API_URL=${process.env.JOLLY_TEST_SALEOR_API_URL}`,
      `SALEOR_APP_TOKEN=${process.env.JOLLY_TEST_SALEOR_APP_TOKEN}`,
      "",
    ].join("\n"),
  );
...
Then(
  ...
  function (this: JollyWorld) {
    const endpoint = process.env.JOLLY_TEST_SALEOR_API_URL as string;
...
Then(lit("the config should use the stored app token"), function (this: JollyWorld) {
  ...
  const token = process.env.JOLLY_TEST_SALEOR_APP_TOKEN as string;
