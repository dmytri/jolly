...
// --- Scenario: jolly start makes the Vercel project turnkey for a plain -----
//     re-deploy (@logic) -------------------------------------------------------

Then(
  "the `deploy` stage preview should name persisting the storefront build env vars `NEXT_PUBLIC_SALEOR_API_URL` and `NEXT_PUBLIC_DEFAULT_CHANNEL` on the Vercel project itself through the Vercel CLI, not only as one-off `--build-env` flags on Jolly's own deploy",
  function (this: JollyWorld) {
    const plan = (this.envelope.data.plan ?? []) as Array<{ stage?: string }>;
    const deploy = plan.find((s) => s.stage === "deploy");
    assert.ok(deploy, "the start --dry-run plan must include the deploy stage");
    const blob = JSON.stringify(deploy).toLowerCase();
    assert.ok(
      /next_public_saleor_api_url/.test(blob) && /next_public_default_channel/.test(blob),
      `the deploy stage preview must name both build env vars: ${blob}`,
    );
    assert.ok(
      /vercel project/.test(blob) && /vercel cli/.test(blob),
      `the deploy stage preview must name persisting the env vars on the Vercel project itself through the Vercel CLI: ${blob}`,
    );
    assert.ok(
      !/only.*(one-off|--build-env flag)/.test(blob) || /not only/.test(blob),
      `the deploy stage preview must state this is not only one-off --build-env flags: ${blob}`,
    );
    assert.ok(
      /not only/.test(blob) && /--build-env/.test(blob),
      `the deploy stage preview must contrast persisted project env vars against one-off --build-env flags on Jolly's own deploy: ${blob}`,
    );
  },
);

Then(
  "the preview should name writing `NEXT_PUBLIC_DEFAULT_CHANNEL` to `.env`, so a plain `npx vercel deploy` and the local storefront both read the store channel with no key juggling",
  function (this: JollyWorld) {
    const plan = (this.envelope.data.plan ?? []) as Array<{ stage?: string }>;
    const deploy = plan.find((s) => s.stage === "deploy");
    assert.ok(deploy, "the start --dry-run plan must include the deploy stage");
    const blob = JSON.stringify(deploy).toLowerCase();
    assert.ok(
      /writ(e|es|ing).*next_public_default_channel.*\.env|next_public_default_channel.*\.env/.test(blob),
      `the deploy stage preview must name writing NEXT_PUBLIC_DEFAULT_CHANNEL to .env: ${blob}`,
    );
    assert.ok(
      /no key juggling|both read the store channel|plain.*vercel deploy/.test(blob),
      `the deploy stage preview must explain a plain vercel deploy and the local storefront both read the channel with no key juggling: ${blob}`,
    );
  },
);
