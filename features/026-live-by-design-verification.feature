Feature: Live-by-design verification conformance
  ...
  Scenario: The eval seeds only authentication credentials, never a pre-provisioned store
    ...
    And it should omit the store endpoint `NEXT_PUBLIC_SALEOR_API_URL` and the `SALEOR_TOKEN`, so a baseline agent's `jolly start` provisions a fresh `jolly-test` store on the real creation path instead of reusing a pre-seeded one
