...
// the candidate paths below (or pointed to by JOLLY_TEST_HOMEPAGE_HTML), and
// the agent setup guide must be a sibling document at one of the guide
// candidates (or JOLLY_TEST_SETUP_GUIDE). A build step is fine as long as the
...
export function homepagePath(): string {
  return resolveFirst(HOMEPAGE_CANDIDATES, process.env.JOLLY_TEST_HOMEPAGE_HTML, "Jolly homepage HTML");
}

export function setupGuidePath(): string {
  return resolveFirst(SETUP_GUIDE_CANDIDATES, process.env.JOLLY_TEST_SETUP_GUIDE, "Jolly agent setup guide");
}
