...
- **new-pass1 1/4 DONE.** 014 "Doctor names the authenticated Vercel account" landed (`da29de3`): `vercel-auth` pass now names the account `vercel whoami` reports (was status-only). Scenario gated `[]` creds + Vercel CLI capability (skip-not-fail without a session). **3 remain:** 018 headless-listener warning (`018:200`), 006 `--help` usage, 002 `start --dry-run` idempotency.
  - **Verification-layer fix (same commit, QM):** feature 026 "no forbidden double" was RED before this pass — the 018 `.invalid` exceptional-double (from `1341a5c`) sat 4 lines from its `@exceptional-double` marker, outside the 026 scanner's 3-line window, so it read as forbidden. Moved the annotation adjacent; comment-only. Logic tier now 0-failed.
  - **Spec-quality, defer to cycle-end Bosun:** 014 now has two live-session `@sandbox` vercel-auth scenarios — "Doctor confirms the Vercel CLI login state when a session exists" (asserts the `vercel whoami` delegation mechanism + pass) and the new "names the account" (pass + account). Distinct observables, both green; consider consolidating when the cycle is swept.
- **Push STILL HELD (dk, 2026-06-18):** push the whole cycle once all four ergonomics land. All commits (`af1319d`+`c24326b`+`1341a5c`+`da29de3`+notes) stay local on `main`.
- **Next (dk, 2026-06-18):** run the remaining 3 new-pass1 targets. Clear context → `/qm`.
