...
- **OBSERVATION — 2 pre-existing `@sandbox` failures under the local `.env` real creds (NOT this cycle's work):** the default-profile run (all tags) shows `002:66` (whole-flow stages all `"completed"`) and `002:77` (`saleor-connectivity` `"pass"`) failing. Neither touches the changed dry-run/`--help`/shared-When paths — mechanically not caused by `2b2845c`. They run (not skip) because a local `.env` carries 3 real credential lines but lacks a full deploy/connectivity capability. **Possible skip-not-fail gating gap** (a `@sandbox` whole-flow/connectivity scenario should skip without the `vercel login` session / reachable store rather than fail) — route to a `@sandbox` harness sweep; the `@logic` worklist is unaffected and green. Not a pass1 target.
- **Push condition MET (dk, 2026-06-18):** dk's standing instruction was "push the whole cycle once all four ergonomics land." All four landed. Cycle commits local on `main` (`af1319d`→`c24326b`→`1341a5c`→`da29de3`→`2b2845c`+notes). Captain offered the push to dk pending go-ahead (the `@sandbox` observation above is a new fact since the instruction).
- **Next:** dk's push call. Then the deferred sandbox sweep + `-p eval` run.
