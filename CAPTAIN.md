...
**Residual — next iteration (isolated, churny):** `026` no-forbidden-double scanner is under-inclusive (misses in-process loopbacks, `STAND_IN_TOKEN`, `--mock-organizations`; justifies by comment not scenario tag) — hardening forces a per-double Captain decision (tag `@exceptional-double` vs make-real; `STAND_IN_TOKEN` on `@logic` may violate AGENTS.md no-forced-safe-creds), so it ships as its own release. Plus two `012` `@sandbox` fidelity gaps (`--url` org/env inference unimplemented + test-masked; domain-collision reject path short-circuited by idempotent reuse).

Prior **0.10.6** (`main` + tag `v0.10.6`, `@dk/jolly` npm `latest`): **`/tokens` removed from every offered path — device authorization grant is the only sign-in Jolly directs to.** `cloud.saleor.io/tokens` no longer appears in any remediation, next-step, or doctor check; every credential-missing/invalid/rejected/no-org branch now steers to `jolly login` (device grant). `JOLLY_SALEOR_CLOUD_TOKEN` stays supported as an **externally-set env for tests/CI only** (the `@sandbox` harness needs it) — never advertised, never a `/tokens` prompt. The fresh-VM culprit was an empty `JOLLY_SALEOR_CLOUD_TOKEN` hitting `jolly login`'s empty-token branch, which used to cite `/tokens`; it now offers the device grant. Specs 003/014/018 + step defs reworded; `TOKEN_PAGE` consts deleted; 014 doctor scenarios now assert device-grant guidance and the *absence* of the tokens page.

Prior **0.10.5** (`main` + tag `v0.10.5`, `@dk/jolly` npm `latest`): **agent-path OSC 8 leak fixed.** `runDeployStage` (`src/index.ts:3277`) wrapped the Vercel sign-in URL in OSC 8 unconditionally — so `jolly start --json` (agent path) emitted escape bytes into agent logs, violating 027's Rule ("clickable where the terminal supports it, plain otherwise"). Now gated on `process.stderr.isTTY`: clickable on an interactive TTY, plain on the agent/`--json` path — mirroring the Saleor interactive-grant vs. agent-relay split. Made falsifiable: `002:113` (`@sandbox`) now asserts the agent run carries no OSC 8 escape, and `018:25` (`@logic`) asserts the Saleor relay is plain (closing the prior substring-only gap that an OSC 8 wrap would have passed).
