...
- **LIVE-BY-DESIGN SPEC SCAFFOLDING LANDED + PUSHED (2026-06-15, `3cd5ddf`, origin/main synced).** Spec-only checkpoint, driven entirely by good specs (no ad-hoc QM direction): feature **026** conformance `@property` "no forbidden double" (the de-fake keystone — makes the no-fakes rule a discoverable red target); `@exceptional-double` tags on the two admissible doubles (**012** env-limit, **018** Cloud-API-unreachable); doctor **014** `vercel-auth` check via `vercel whoami` (logic absence + sandbox presence) + single-oracle delegation principle; **020** Stripe-mirrors-Vercel (dropped `api.stripe.com` from Jolly's request allowlist — vestigial; Stripe reached only via the spawned CLI); rule refinement (AGENTS.md/SCENARIO_WRITING.md/CAPTAIN.md — *testability, not subject*, admits conformance invariants). **QM worklist = the 4 undefined `--dry-run` targets**: 026 conformance, 014 vercel-auth ×2 (logic+sandbox), 020 first-party-host re-aim. No release: `src` unchanged, suite intentionally spec-ahead. The delegation principle largely *codified* existing design (005 already imports Stripe keys read-only via the CLI; Vercel already CLI-only). Next: fresh session → `/qm` drives the harness rewrite from these reds (then the centralize-the-env-seam refactor, the @eval live-URL work, and the launch-bar pass remain — see below).
